...
function renderSuppliers() {
  ...
  if (filtered.length === 0) {
    grid.innerHTML = '<div class="col-span-2 text-center py-8 text-xs font-label-mono text-on-surface-variant">No matching items found in commercial linked accounts.</div>';
    return;
  }
  filtered.forEach(item => {
    ...
    grid.innerHTML += `<div class="hud-border p-4 text-left group hover:bg-brand-purple/5 transition-colors flex flex-col justify-between"><div><div class="flex justify-between items-start mb-2"><span class="font-label-mono text-[9px] text-brand-blue uppercase tracking-wider">${item.category.toUpperCase()}</span><span class="font-label-mono text-[9px] px-1.5 py-0.5 rounded-sm bg-${statusColor}-500/10 text-${statusColor}-400">${item.stockStatus}</span></div><h5 class="font-medium text-xs text-white uppercase tracking-wide mb-3 min-h-[32px] line-clamp-2">${item.name}</h5></div><div class="flex items-end justify-between mt-2 pt-2 border-t border-outline-variant/30"><div><span class="font-label-mono text-[10px] text-on-surface-variant/70">SUPPLIER VALUE</span><p class="font-headline-lg text-lg text-white">$${item.price.toFixed(2)} <span class="text-[10px] text-on-surface-variant">/${item.unit}</span></p></div><div class="flex items-center gap-2">${plotted > 0 ? '<span class="font-label-mono text-[10px] bg-brand-blue/15 text-brand-blue px-2 py-1 rounded-sm border border-brand-blue/30">Active: ' + plotted + '</span>' : ''}<button ${item.stockStatus === 'OUT OF STOCK' ? 'disabled' : ''} onclick="procureItem('${item.id}','${item.name.replace(/'/g, "\\'")}',${item.price},'${item.category}','${item.unit}')" class="bg-brand-blue/90 hover:bg-brand-purple disabled:bg-gray-800 disabled:text-gray-500 text-white p-2 rounded-sm transition-colors flex items-center gap-1 text-[10px] font-label-mono uppercase tracking-wider"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>Procure</button></div></div></div>`;
  });
}

function procureItem(id, name, price, category, unit) {
  const item = { id: Math.random().toString(36).substr(2, 9), name, category, quantity: 1, unitPrice: price, unit };
...
function set3DLayer(layer) {
  ...
  document.querySelectorAll('#layer-electrical, #layer-plumbing, #layer-structural').forEach(b => {
    if (b.id === 'layer-' + layer) {
      b.className = 'px-2 py-1 text-[8px] font-label-mono rounded-sm border uppercase transition-all cursor-pointer ' +
      ...
      b.className = 'px-2 py-1 text-[8px] font-label-mono rounded-sm border uppercase transition-all cursor-pointer border-white/5 hover:border-white/10 text-slate-400 bg-black/40';
    }
  });
  renderGrid();
}
